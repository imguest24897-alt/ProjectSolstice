	      ...
	      struct gpiol_desc *desc, uint32_t flags)
{
	...
	if (name) {
		name_len = strnlen(name, LK2ND_GPIOL_PROP_LEN + 1);
		if (name_len > LK2ND_GPIOL_PROP_LEN) {
			dprintf(CRITICAL, "gpiol_get failed: property name is too long\n");
			return -1;
		}

		memcpy(prop_name, name, name_len);
		memcpy(prop_name + name_len, "-gpios", sizeof("-gpios"));
	}
...
{
	desc->active_low = !!(config & GPIO_ACTIVE_LOW);
